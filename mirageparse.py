...
import datetime
from parse import StatusParser
from sensor import *
import include
...
class MirageStatusParser(StatusParser):
    ...
    FYI vim has issues with highlighting the below for some reason.
    message format(bash):
	...
	@param	status_string	The contents of the sensor status upload
        @param	configs		Keyword arg configs
	'''
	...
	'''
        ...
        try:
	    # turn the date string into a datetime instance and add a year field 
            # because it is missing from the data from the sensor
